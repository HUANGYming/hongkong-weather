...
docker exec "$HKO_POSTGRES_CONTAINER" psql -U hko -d hko_weather \
  ...
  -c "select obs_time, fetched_at, source, metric, value from fact_feature_observation_hkweather_10min_realtime_v1 order by obs_time desc, source, metric limit 10;"
