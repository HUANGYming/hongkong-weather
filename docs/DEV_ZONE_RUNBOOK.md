...
The Python scripts load `.env` automatically for manual/local runs. Airflow Docker mode does not read the project `.env`; Docker build copies `.env` into the image as `/app/.env`.
